...
try:
    ...
    ac.console("gui or settings err")
...
def acMain(ac_version):
    ...
    app_win = gui.App("ACBIB", "", 200, 200, gui.Color(0, 0, 0, 0.8))

    la = ac.addLabel(app_win.app, "as")
    ac.setPosition(la, 20, 20)

    rpm = gui.Label(app_win, app_win, "test")
    ac.setPosition(rpm.ac_obj, 20, 20)

    l_lapcount = ac.addLabel(app_win.app, "Laps: 0")
...
def acUpdate(deltaT):
    ...
    global rpm
    global la
    global lapcount, l_lapcount

    app_win.update()

    rpm.setText(acbib.ACCAR.getRPM())
    ac.setText(la, acbib.ACCAR.getRPM())
